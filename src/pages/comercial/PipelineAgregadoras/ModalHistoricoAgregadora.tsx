...
import type { AgregadoraHistorico, AgregadoraRow } from "./types";
import { fmtDataHora, historicoDisplayValorAgregadora } from "./helpers";
import { HISTORICO_CAMPO_LABEL_AGREGADORA } from "./constants";

export function ModalHistoricoAgregadora({
  agregadora,
  onClose,
}: {
  ...
  const load = useCallback(async () => {
    ...
    const { data, error } = await supabase
      ...
      .eq("agregadora_id", agregadora.id)
  ...
  return (
    <ModalBase onClose={onClose} maxWidth={560} zIndex={1000}>
      <ModalHeader title={`Histórico — ${agregadora.nome}`} onClose={onClose} />
      {loading ? (
          ...
          {itens.map((h) => (
            ...
            >
              <div style={{ fontWeight: 700, marginBottom: 4 }}>
                {HISTORICO_CAMPO_LABEL_AGREGADORA[h.campo] ?? h.campo}
              </div>
              <div style={{ color: t.textMuted, fontSize: 12 }}>
                {historicoDisplayValorAgregadora(h.campo, h.valor_anterior)}
                {" → "}
                {historicoDisplayValorAgregadora(h.campo, h.valor_novo)}
              </div>
