...
const PERIODO_LABEL: Record<string, { pt: string; en: string }> = {
      ...
      <div>
        ...
        {dayLives.length === 0 ? (
          <div style={{ textAlign: "center", color: t.textMuted, fontSize: "14px", padding: "40px 0", fontFamily: FONT.body }}>
            {lang === "en" ? "No lives scheduled for this day." : "Nenhuma live agendada para este dia."}
          </div>
        ) : (
              ...
              <div key={l.id} onClick={() => setModal({ open: true, live: l })}
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: "flex", gap: "8px", marginTop: "6px", flexWrap: "wrap" }}>
                    ...
                    <span style={{ fontSize: "11px", color: t.textMuted, fontFamily: FONT.body }}>🕐 {l.horario.slice(0,5)} · {PERIODO_LABEL[l.periodo][lang]}</span>
                  </div>
                </div>
              </div>
            ))}
    ...
    <div style={{ padding: "24px", maxWidth: "1100px", margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "20px", flexWrap: "wrap", gap: "12px" }}>
        <h1 style={{ fontSize: "22px", fontWeight: 900, color: t.text, fontFamily: FONT.title, margin: 0 }}>
          {lang === "en" ? "Live Schedule" : "Agenda de Lives"}
        </h1>
        {isAdmin && (
      ...
      <div style={{ ...card, marginBottom: "16px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "12px", marginBottom: "16px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            ...
            <button onClick={goToday} style={chip(false)}>{lang === "en" ? "Today" : "Hoje"}</button>
          </div>
          <div style={{ display: "flex", gap: "6px" }}>
            {(["mes","semana","dia"] as ViewMode[]).map(v => (
              <button key={v} onClick={() => setView(v)} style={chip(view === v)}>
                {v === "mes" ? (lang === "en" ? "Month" : "Mês") : v === "semana" ? (lang === "en" ? "Week" : "Semana") : (lang === "en" ? "Day" : "Dia")}
              </button>
            ))}
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
          <input value={search} onChange={e => setSearch(e.target.value)}
            placeholder={lang === "en" ? "Search live or influencer..." : "Buscar live ou influencer..."}
          ...
          <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)}
            ...
            <option value="todos">{lang === "en" ? "All statuses" : "Todos os status"}</option>
          ...
          <select value={filterPlat} onChange={e => setFilterPlat(e.target.value)}
            style={{ padding: "8px 12px", borderRadius: "10px", border: `1px solid ${t.inputBorder}`, background: t.inputBg, color: t.inputText, fontSize: "13px", fontFamily: FONT.body, cursor: "pointer" }}>
            <option value="todas">{lang === "en" ? "All platforms" : "Todas as plataformas"}</option>
