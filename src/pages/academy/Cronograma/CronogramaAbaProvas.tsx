...
import type { AcademyProva, AcademyTrilha } from "../../../lib/academyCronogramaTypes";
import { fmtDataCatalogo, nomesVinculados, trilhasDaProva } from "../../../lib/academyCronogramaUi";
...
export function CronogramaAbaProvas({
  brand,
  t,
  rows,
  trilhas,
    ...
    <div style={getPageContentBoxStyle(brand, t)}>
      ...
      <div className="app-table-wrap--sticky-col" style={getDataTableWrapStyle()}>
        <table style={getDataTableStyle()}>
          <caption className="sr-only">Provas de treinamento</caption>
          <thead>
            <tr>
              <th scope="col" style={dt.thHeaderSticky}>Prova</th>
              <th scope="col" style={dt.thHeader}>Trilha</th>
              <th scope="col" style={dt.thHeader}>Questões</th>
              <th scope="col" style={dt.thHeader}>Nota mínima</th>
            ...
            {linhasPagina.length === 0 ? (
              ...
              linhasPagina.map((row, i) => (
                <tr key={row.id} style={{ background: dt.zebraRow(zebraIdx(i)) }}>
                  ...
                  <td style={dt.tdCenter}>{nomesVinculados(trilhasDaProva(row.id, trilhas, trilhaProvas).map((tr) => tr.nome))}</td>
                  <td style={dt.tdCenter}>{row.questoes.length}</td>
                  <td style={dt.tdCenter}>{`${row.nota_minima}%`}</td>
