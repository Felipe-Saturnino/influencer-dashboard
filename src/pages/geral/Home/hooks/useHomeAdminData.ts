 ...
 * Operadoras ativas = mesmas linhas Ativa da Gestão de Operadoras (`operadoras.ativo`).
 */
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        const [ggrDed, ggrNet, falhas, ops, users, prest] = await Promise.all([
          ...
          // Gestão de Operadoras: Status Ativa = `ativo === true` (PK = slug, não id)
          supabase.from("operadoras").select("slug", { count: "exact", head: true }).eq("ativo", true),
        ...
        if (cancelled) return;
        if (ops.error) {
          console.error("[HomeAdmin] operadoras:", ops.error);
          setErro(true);
          return;
        }
