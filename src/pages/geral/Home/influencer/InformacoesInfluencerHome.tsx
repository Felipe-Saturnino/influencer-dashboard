...
/** Informativos do perfil influencer — janela 10 dias + «Li e Ocultar»; sem Portal RH / Academy. */
export function InformacoesInfluencerHome({ sectionIdPrefix = "home-influencer" }: { sectionIdPrefix?: string }) {
  const { theme: t } = useApp();
  const { userId: userIdEfetivo } = useIdentidadeEfetiva();
  const brand = useDashboardBrand();
  const publicadoDesdeIso = useMemo(() => getHomeStaffFeedNovidadeDesdeIso(), []);
  const info = useHomeInformativos("influencer", { publicadoDesdeIso });
