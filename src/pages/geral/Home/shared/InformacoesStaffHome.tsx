...
import {
  useHomeStaffInformacoesFeed,
  type HomeStaffInformacaoItem,
} from "../hooks/useHomeStaffInformacoesFeed";
...
function DescricaoGaleriaCard({ item }: { item: HomeStaffInformacaoItem }) {
...
export function InformacoesStaffHome({
  perfil,
  sectionIdPrefix,
  includeGaleriaNovidades = true,
}: {
  perfil: Role;
  sectionIdPrefix: string;
  includeGaleriaNovidades?: boolean;
}) {
  const { theme: t } = useApp();
  const brand = useDashboardBrand();
  const { loading, erro, lista } = useHomeStaffInformacoesFeed(perfil, { includeGaleriaNovidades });
