...
import { useHomeInvestidorInformativos } from "../hooks/useHomeInvestidorInformativos";
import { homeInvestidorSectionTitleStyle, HOME_INVESTIDOR_BODY_MUTED } from "./homeInvestidorUi";

export function InformativosInvestidor() {
  const { theme: t } = useApp();
  const brand = useDashboardBrand();
  const { loading, erro, lista } = useHomeInvestidorInformativos();
  const box = getPageContentBoxStyle(brand, t);
  const cardShadow = getPageContentBoxShadow(t.isDark ?? false);

  return (
    <section style={box} aria-labelledby="home-investidor-info-title">
      <h2 id="home-investidor-info-title" style={homeInvestidorSectionTitleStyle(t.sectionTitle)}>
        Informações
      </h2>
