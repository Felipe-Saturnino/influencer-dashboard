  ...
  streamers: {
    ...
    blocos: [
      ...
      },
      {
        ...
        texto:
          "Os cadastros por jogador (ID Ext + UTM) entram no mesmo sync da Casa de Apostas que atualiza as métricas consolidadas. A aba Jogadores conta o daily do mês no carrossel — não o acumulado lifetime do cadastro. **Todos Influencers** e **Todas Operadoras** respeitam o seu escopo: com permissão de Ver **próprios**, os KPIs são só dos seus cadastros e das suas casas — não da plataforma. Se Overview e Conversão têm números mas Jogadores não, o sync agregado rodou e a fase de IDs falhou — em geral falta permissão de relatório de registros na TAP. “Jogaram Spin” exige **rodada Spin > 0** no Revenue Sentinel (`round_count` / `bet_count`); só existir no lake do RS não conta. Se o Status Técnico mostrar o sync **OK** com 0 registros no dia, republicar a Edge `sync-revenue-sentinel` (parser incluso, v1.3.0+) e rodar o Sync de novo. Enquanto os dias com rodada não gravarem em `jogadores_metricas_diarias`, os KPIs de mesa ficam em 0 e os registros TAP do período continuam visíveis. Se o problema persistir, entre em contato com o suporte.",
      },
      {
        subtitulo: "Os KPIs mostram valores diferentes entre Overview e Financeiro?",
        texto:
          "As quatro abas usam a mesma busca linha a linha de métricas e lives do período (Overview, Conversão e Financeiro) ou o cadastro diário de jogadores (aba Jogadores). Diferenças entre Overview e Financeiro costumam vir do recorte exibido (ex.: investimento de agentes no Overview, PVI e perfil de jogador no Financeiro), não de um procedimento especial do banco. O GGR da aba Jogadores é o das mesas Spin, não o GGR TAP. Se a divergência parecer inconsistente para o mesmo filtro e mês, entre em contato com o suporte informando o mês e os valores observados.",
      },
    ],
  },
  dash_afiliados: {
