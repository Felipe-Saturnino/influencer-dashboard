  ...
  escala_controle_turno: {
    ...
    blocos: [
      ...
      },
      {
        subtitulo: "Rotação",
        texto:
          "O pool usa a Escala Estúdio do estúdio selecionado, restrito aos prestadores da **Escala do Turno** com status **Presente**, **Pendente**, **Saída Antecipada** ou **Hora Adicional**. Com horário de saída registrado, a grade marca **X** a partir desse horário. **Hora Adicional** do turno anterior também entra no turno seguinte (mesmo estúdio). **Falta** fica de fora do pool. Na prévia, use **Trazer de outro estúdio** (ou **Mover estúdio** no chip) para cobrir mesas com GPs escalados noutro estúdio no mesmo turno — a alocação vale o turno inteiro. Na **Pré-visualização**, arraste o nome na coluna **Equipe** para trocar a sequência de mesas com outro prestador (ex.: Maria passa a começar na mesa que era da Amanda). O botão **Rotação de 20min** regenera a grade em intervalos de 20 minutos e passa a **Rotação de 30min** para voltar ao padrão. **Shift Leaders** e **Service Managers** **não** entram no Pool sozinhos — só pelo botão **Incluir Liderança**. Após incluir, a grade marca **X** fora da janela **08h–20h** / **20h–08h** quando o turno só cobre parte do horário.",
      },
      {
