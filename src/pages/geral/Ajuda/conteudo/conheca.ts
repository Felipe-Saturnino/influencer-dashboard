  ...
  streamers: {
    ...
    blocos: [
      ...
      },
      {
        ...
        texto:
          "Apresenta os KPIs executivos agrupados em três blocos: Financeiro (GGR, Investimento, ROI), Operação (Influencers — Ativos/Inativos, Lives — Quantidade/Horas, Depósitos — Quantidade/Volume) e Conversão (Registros — Quantidade/Custo Médio, FTDs — Quantidade/Custo Médio, UAP Spin — Quantidade/Rodadas).\n\nInfluencers considera somente quem fez live realizada no período. Ativos e Inativos usam o status atual do cadastro; Inativo inclui também Cancelado. Ao consultar um mês passado ou o Histórico, a classificação continua refletindo o status atual, pois não existe série histórica de status.\n\nUAP Spin é a quantidade de jogadores TAP únicos, mapeados ao canal de influencer, que tiveram rodada em mesa Spin no período. Rodadas soma o volume desses jogadores. No Histórico, o mesmo TAP ID é contado uma vez em toda a janela, ainda que tenha jogado em vários meses. Os Registros desta aba também são IDs Ext únicos das UTMs mapeadas a influencer — o mesmo número da aba Jogadores, sem somar o mesmo cadastro duas vezes.\n\nO Funil de Conversão exibe as taxas de conversão do canal: de Views até FTDs.\n\nO Ranking de Influencers lista todos os influencers com dados no período com status de performance (Rentável, Atenção, Não Rentável, Bônus, Sem dados). Use os badges de status no canto superior direito para filtrar por categoria. Clique nos cabeçalhos das colunas para ordenar. Em listas longas (ex.: Histórico), a tabela pagina a vista — os totais dos KPIs continuam do conjunto completo.",
      },
      {
      ...
      },
      {
        ...
        texto:
          "Responde quem o canal cadastrou e quem de fato jogou nas mesas Spin. Os cadastros vêm da TAP (ID Ext da casa + UTM do influencer). Rodadas, GGR Spin, turnover e mesas vêm do cruzamento com o Revenue Sentinel.\n\nNo carrossel mensal, cadastros e atividade em mesa pertencem ao mês selecionado. No Histórico, as competências são somadas. A integração consulta uma competência por vez e usa a data real das rodadas enviada pelo RS.\n\nO primeiro bloco divide os registros únicos das UTMs mapeadas em três grupos sem sobreposição: Jogaram Spin (rodada Spin no mês), Jogaram Outros (depósito no mês e nenhuma rodada Spin) e Não Jogaram (sem depósito e sem rodada Spin). A soma dos três fecha o total de registros únicos. A taxa de ativação é Spin ÷ Registros. O GGR desta aba é o das mesas Spin, não o da TAP.\n\nO Comparativo de Funil coloca dois influencers lado a lado em três etapas (Registros → Jogaram → Jogaram Spin). Rodadas por Mesa cruza o ID da mesa no RS com a Gestão de Estúdios e exibe rótulos como **Dedicada — Blackjack 1** e **Network — Roleta**; somente mesas ainda não mapeadas aparecem como “Mesa não informada”. O Ranking mostra até os 10 influencers com mais rodadas no período. No Comparativo de Taxas mensal, influencers sem registros no mês não aparecem.\n\nOs filtros de período, influencer e operadora são os mesmos das outras abas. No modo Histórico o comparativo MoM some e os subtítulos passam a \"acumulado\".",
      },
    ],
  },
  mesas_spin: {
  ...
  rh_calendario: {
    ...
    blocos: [
      ...
      },
      {
        ...
        texto:
          "O Check-in e o Check-out podem ser registrados mesmo quando o dia estiver como **Folga** ou **Venda**, inclusive em fins de semana e plantões emergenciais — **exceto** para **Game Presenter** e **Shuffler**, que só registram em dia trabalhado (Escalado, Compra ou Troca) e dentro da janela de **15 minutos** antes do início / após o fim do turno (turno efetivo = Hora Adicional do Controle de Turno, quando houver, ou horário da escala). Fora da janela, a plataforma pede que a Liderança registre no Controle de Turno.\n\nA Situação permanece Folga/Venda nos demais times; Entrada e Saída realizadas ficam registradas para aprovação posterior do gestor. Dias com **Compra** (ou Compra - Turno) seguem as mesmas regras de **Escalado** no Controle de Presença (Falta, Justificar e Aprovar).\n\nNa aba **Controle de Presença**, cada linha é o turno do dia — não o dia civil do relógio. Se o check-in for à noite (ex.: 20h) e a saída na manhã seguinte (ex.: 08h), o check-out fica na mesma linha do dia do check-in. O botão **Fazer Check-out** permanece disponível por 20 horas após o check-in (demais times).\n\nCorreções de horário (**Esquecimento**, **Outro** ou Correção) ficam **Em análise**: o líder imediato aprova **entrada e saída separadamente** no ícone de comentário de cada coluna. O próprio prestador não aprova a própria correção. Justificativa **Médico** segue para **Solicitações** (RH).",
      },
      {
      ...
      {
        subtitulo: "Relatório de Justificativas",
        texto:
          "Aba disponível para quem tem **Criar: Sim** (visão empresa), ou **Ver: Sim** e **Editar: Sim** (recorte do Organograma), ou Administrador. Com **Meu Calendário** ativo a aba some. Quem tem só **Ver/Editar: Próprios** não vê esta aba. Use o carrossel de **mês** e os filtros de **Time** e **Staff** (um de cada vez) para listar justificativas **Em análise** no mês. Selecione ao menos um dos filtros para carregar. Atestado médico de **vários dias**: confira também o período completo no **Controle de Presença** do mês.",
      },
    ],
  },
  escala_marketplace_turnos: {
    ...
    blocos: [
      ...
      },
      {
        ...
        texto:
          "Com **Ver: Sim** e **Minhas Negociações** desligado, a aba **Ofertas Spin** concentra coberturas e liberações publicadas pela **Spin Gaming**. **Nova Oferta** nesta aba abre o modal operacional: **Time** (Game Presenter, Shuffler ou Liderança), **Cobertura de turno** ou **Liberação de vaga**, dia, turno, **quantidade** (padrão 1 — várias unidades geram várias linhas em **Todas as Ofertas**), estúdio (só Game Presenter — Shuffler e Liderança gravam **Todos Estúdios**) e observação. Publicar exige **4h** até o início do turno; aceite no mural atualiza a escala **na hora** (Compra - Turno ou Venda) — sem proposta Em análise. Coberturas entram no mural em **Ofertas de Turno**; liberações em **Ofertas de Folga**. A aba organiza **Ofertas abertas**, **Ofertas aceitas**, o bloco **Compra Turno / Compra Folga** (vendas P2P de GP e Shuffler — gestão propõe compra em nome da Spin Gaming; ofertante aprova em **Minhas Ofertas**; proposta Em análise permanece no bloco com **Desistir da proposta**) e **Histórico** com filtro de **Status** (padrão **Aprovada**; com o botão Histórico da barra, últimos 30 dias). Tutorial: `/Ajuda/Tutoriais/MarketplaceOfertasSpin`.",
      },
    ],
  },
  ...
  tech_ops_ordem_saida: {
    ...
    blocos: [
      ...
      },
      {
        ...
        texto:
          "O carrossel cobre o mês atual e os dois anteriores. O botão Histórico mostra todo o período. Ordens ainda Solicitadas ou Abertas de meses anteriores continuam aparecendo no mês seguinte até serem Concluídas ou Canceladas.\n\nOs cards Total, Solicitadas, Abertas, Concluídas e Canceladas filtraram a listagem por status — clique de novo no card ativo (ou em Total) para limpar.\n\nCada tabela mostra até **20 linhas** por vez. Com mais ordens, use as setas abaixo da tabela (a barra some quando cabe numa página). Os cards de status continuam no conjunto completo.",
      },
      {
        subtitulo: "Códigos e solicitação",
        texto:
          "Cada OS recebe um código automático no formato OS/INT-MMAA-####, OS/EXT-MMAA-#### ou OS/MAN-MMAA-#### (mês/ano de abertura + sequencial). Ao Solicitar, a ordem entra como Solicitada (na Manutenção o rótulo exibido é Registrada).\n\nNa aba **Itens da OS**, o campo Item abre uma lista com busca (quando há muitos cadastros), agrupada em Itens, Equipamentos e Jogo — digite parte do código ou nome para filtrar.",
      },
      {
        subtitulo: "Ver, Aprovar e Atualizar",
        texto:
          "No Ver, o título é o código da OS e o subtítulo mostra status e responsável (Interna) ou solicitante (Externa/Manutenção). A aba Dados lista os campos do fluxo e o bloco de itens; Cancelada e Concluída podem exibir quadros de cancelamento ou conclusão. Anotações inclui a observação de abertura; Histórico registra as ações.\n\nQuando a OS está Solicitada, Aprovar muda o status para Aberta (ou Concluída quando for Sem retorno) e atualiza a **Gestão de Estoque**: equipamentos passam a **Em uso** (ou **Manutenção** na O.S. de Manutenção) e a quantidade Em uso dos itens sobe. Recusar exige o Motivo do Cancelamento e muda a OS para Cancelada.\n\nNo Atualizar, escolha o tipo: Cancelar OS (motivo → Cancelada; se estava Aberta, devolve ao estoque), Confirmar Retorno (data de hoje, checkboxes por item e observações → Concluída e devolve ao estoque) ou Alterar OS (edita dados e itens). Em OS ainda abertas/solicitadas, Alterar volta a Solicitada. Em OS **Concluída sem retorno**, o botão Atualizar continua disponível só com Alterar OS — as mudanças ficam no Histórico e o estoque é reajustado. As opções disponíveis dependem da aba e do status.",
      },
      {
        subtitulo: "Permissões",
        texto:
