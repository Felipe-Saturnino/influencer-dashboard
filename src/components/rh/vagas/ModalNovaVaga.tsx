...
export function ModalNovaVaga({
  ...
  async function salvar() {
    ...
    setSalvando(false);
    if (error) {
      console.error("Vagas: falha ao criar", error);
      setErroSalvar("Não foi possível salvar a vaga. Se o problema persistir, entre em contato com o suporte.");
      return;
    }
    onSalvo();
    onClose();
  ...
  return (
    <ModalBase maxWidth={560} onClose={fechar} zIndex={1100} panelOverflow="hidden">
      <ModalHeader title="Nova vaga" onClose={fechar} />
      <div style={MODAL_FORM_SHELL_STYLE}>
        <div style={{ ...MODAL_FORM_SCROLL_BODY_STYLE, fontFamily: FONT.body }}>
          {erroOrg ? <div style={{ fontSize: 13, color: "#e84025", fontFamily: FONT.body }}>{erroOrg}</div> : null}
          {carregandoOrg ? (
          ...
          {erroSalvar ? (
            <div role="alert" style={{ fontSize: 13, color: "#e84025", fontFamily: FONT.body }}>
              {erroSalvar}
            </div>
          ) : null}
        </div>
