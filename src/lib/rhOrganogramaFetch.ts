...
export const ORG_DIRETORIA_SELECT =
...
export async function carregarArvoreOrganograma(): Promise<{
  ...
  const err = dr.error || gr.error || tr.error || fr.error;
  if (err) {
    console.error("[rhOrganogramaFetch] carregarArvoreOrganograma:", err);
    return {
      arvore: [],
      nomePorFuncionarioId: new Map(),
      error: "Não foi possível carregar o organograma. Se o problema persistir, entre em contato com o suporte.",
    };
  }
...
export async function carregarOpcoesTimesOrganograma(): Promise<{
  ...
  const err = dr.error || gr.error || tr.error || fr.error;
  if (err) {
    console.error("[rhOrganogramaFetch] carregarOpcoesTimesOrganograma:", err);
    return {
      opcoes: [],
      grupos: [],
      error: "Não foi possível carregar o organograma. Se o problema persistir, entre em contato com o suporte.",
