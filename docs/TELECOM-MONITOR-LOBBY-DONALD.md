# Job Telecom — Monitor de lobby Donald Bet

Documento para a equipe de **Telecom** operar o job horário de posicionamento das mesas Spin na grade **Todos os jogos** da DonaldBet.

O script **só busca a grade** na Donald e envia o JSON para a plataforma. O cálculo de posição e a gravação ficam na **Edge Function** `monitor-lobby-donald`.

**Expansão:** novas mesas **não** exigem alteração deste job. A Spin cadastra o ID Donald em **Gestão de Estúdios**; o próximo ciclo já rastreia.
...
1. Paginar `GET https://donald.bet.br/api/games/category/todos-os-jogos?offset=…&limit=48`.
2. Continuar até achar **todos** os IDs cadastrados na Spin **ou** esgotar o catálogo (`hasMore` / `total`).
3. Enviar o snapshot para a Spin (`monitor-lobby-donald`).
...
| Arquivo | Descrição |
|---------|-----------|
| `scripts/monitor-lobby-donald-run.mjs` | Script Node.js (único ficheiro do job) |
...
| `MONITOR_LOBBY_DONALD_INGEST_SECRET` | Se a Spin configurar | Header `x-monitor-lobby-donald-secret` |
| `DONALD_LOBBY_CATEGORY_URL` | Não | Default: `https://donald.bet.br/api/games/category/todos-os-jogos` |
| `DONALD_LOBBY_CATEGORY` | Não | Default: `todos-os-jogos` |
| `DONALD_LOBBY_LIMIT` | Não | Default: `48` |
...
| Item | Valor |
|------|--------|
| Página | `https://donald.bet.br/cassino/categoria/todos-os-jogos` |
| Request | `GET /api/games/category/todos-os-jogos` |
| Query | `offset` · `limit=48` |
| URL exemplo | `https://donald.bet.br/api/games/category/todos-os-jogos?offset=0&limit=48` |
...
1. Abrir `https://donald.bet.br/cassino/categoria/todos-os-jogos`.
...
| Quem | O quê |
|------|--------|
| **Spin / Data Intelligence** | Gestão de Estúdios → mesa → **ID Donald Bet** = `data[].id` |
...
Operadora no DI: slug **`donald_bet`**.
...
```bash
node scripts/monitor-lobby-donald-run.mjs --dry-run
```
...
```bash
node scripts/monitor-lobby-donald-run.mjs
```
...
| Sintoma | Ação |
|---------|------|
| `Nenhuma mesa com ID Donald` | Spin ainda não cadastrou IDs — avisar Spin |
...
```text
Buscando lobby Donald (4 mesas no cadastro)...
IDs: good-game-v2:live-baccarat, good-game-v2:live-blackjack, …
Donald scan=v1-games-category-offset-all-ids-or-exhaust
GET https://donald.bet.br/api/games/category/todos-os-jogos offset/limit=48
```
