# Job Telecom — Monitor de lobby Rico Bet

Documento para a equipe de **Telecom** operar o job horário de posicionamento das mesas Spin na grade **Todos os jogos** da RicoBet (`/games/category/all`).

O script **só busca a grade** na Rico e envia o JSON para a plataforma. O cálculo de posição e a gravação ficam na **Edge Function** `monitor-lobby-rico`.

**Expansão:** novas mesas **não** exigem alteração deste job. A Spin cadastra o ID Rico em **Gestão de Estúdios**; o próximo ciclo já rastreia.

**Diferença vs Bateu:** a Rico usa `casino-games/filter` (não `/list`) na página `/games/category/all`. Os **IDs das mesas Spin são os mesmos** da Bateu/BRX (Good Game Labs).
...
1. Paginar `GET https://rico.bet.br/api/casino-games/filter?sort=&per_page=24&page=…&term=`.
2. Continuar até achar **todos** os IDs cadastrados na Spin **ou** esgotar o catálogo.
3. Enviar o snapshot para a Spin (`monitor-lobby-rico`).
...
| Arquivo | Descrição |
|---------|-----------|
| `scripts/monitor-lobby-rico-run.mjs` | Script Node.js principal |
...
| `MONITOR_LOBBY_RICO_INGEST_SECRET` | Se a Spin configurar | Header `x-monitor-lobby-rico-secret` |
| `RICO_LOBBY_FILTER_URL` | Não | Default: URL abaixo |
| `RICO_LOBBY_PER_PAGE` | Não | Default: `24` |
...
| URL exemplo | `https://rico.bet.br/api/casino-games/filter?sort=&per_page=24&page=1&term=` |
| Ordem / posição | `(page − 1) × 24 + índice` em `data[]` (1-based) |

### Como validar no DevTools

1. Abrir `https://rico.bet.br/games/category/all`.
...
|------|-----|----------------|
| Roleta | `good-game-v2:live-roulette` | ~P3485 |
| Blackjack | `good-game-v2:live-blackjack` | ~P3486 |
| Baccarat | `good-game-v2:live-baccarat` | ~P3487 |
| Futebol Brasileiro | `good-game-v2:live-cardmatchup` | ~P3488 |

Mesas no **fim** do catálogo (~página 146 de ~152) — timeout alto obrigatório.
...
| Quem | O quê |
|------|--------|
| **Spin / Data Intelligence** | Gestão de Estúdios → mesa → **ID Rico Bet** = `data[].id` |
...
| Sintoma | Ação |
|---------|------|
| `Nenhuma mesa com ID Rico` | Spin ainda não cadastrou — avisar Spin |
