...
describe("diasOfertaveisMarketplace", () => {
  ...
  it("lista só turnos futuros para venda de turno", () => {
    const dias = diasOfertaveisMarketplace("venda_turno", grade, hoje);
    expect(dias.map((d) => d.iso)).toEqual(["2026-08-11", "2026-08-14"]);
    expect(dias[0]!.turno).toBe("Noite");
    expect(dias[0]!.label).toContain("11/08/2026");
  });

  it("lista só folgas futuras para venda de folga", () => {
...
describe("turnosOfertaveisNaFolgaMarketplace", () => {
  it("remove turnos que ficariam a menos de 12h do último turno trabalhado", () => {
    const grade = new Map<string, string>([
      ["2026-08-09", "NGT"],
      ["2026-08-10", "Folga"],
    ]);
    const turnos = turnosOfertaveisNaFolgaMarketplace("2026-08-10", grade, HORARIO_4X2, OPERADORA);
    expect(turnos).not.toContain("Manhã");
    expect(turnos).toContain("Noite");
  });
});
