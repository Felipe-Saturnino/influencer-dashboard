-- Legado Figurinos — Gravata Masculino Único Blaze + CDA (self-contained)
...
--
-- Total: 85 peças (40 Blaze + 45 CDA)
-- Tamanho: Único | Gênero: Masculino | Cor: Único | Entrada: 01/01/2026
-- Códigos: prefixo GRA-* (Gravata)
...
-- ─── Carga legado Gravata Masculino Único (Blaze + CDA) ──────────────────────
...
BEGIN
  ...
  RAISE NOTICE 'Gravata | Tamanho: Único | Masculino | Cor: Único | Data: 2026-01-01';

  FOR v_lote IN
    SELECT estudio_slug::text AS estudio_slug, quantidade::integer AS quantidade
    FROM (VALUES
      ('blaze', 40),
      ('cda',   45)
    ) AS lotes(estudio_slug, quantidade)
  LOOP
    SELECT count(*) INTO v_n
    FROM public.rh_figurino_criar_pecas_lote(
      p_estudio_slugs           := ARRAY[v_lote.estudio_slug]::text[],
      p_category                := 'Gravata'::text,
      p_size                    := 'Único'::text,
      p_purchase_date           := DATE '2026-01-01',
      p_quantidade              := v_lote.quantidade,
      p_actor                   := 'carga-legado'::text,
      p_description             := ('Legado — Gravata Único Masculino Único — ' || upper(v_lote.estudio_slug))::text,
    ...
    IF v_n <> v_lote.quantidade THEN
      RAISE EXCEPTION 'Esperado % peça(s) Gravata (%), cadastradas %', v_lote.quantidade, v_lote.estudio_slug, v_n;
    END IF;

    v_total := v_total + v_lote.quantidade;
    RAISE NOTICE 'Gravata Único Masculino Único (%): % peça(s).', v_lote.estudio_slug, v_lote.quantidade;
  END LOOP;

  RAISE NOTICE 'Carga concluída — % peça(s) Gravata Blaze + CDA (GRA-*).', v_total;
END;
$carga$;

-- ─── Conferência ─────────────────────────────────────────────────────────────
-- SELECT je.estudio_slug, count(*) AS qtd, min(p.code) AS primeiro, max(p.code) AS ultimo
-- FROM public.rh_figurino_pecas p
-- INNER JOIN public.rh_figurino_peca_estudios je ON je.peca_id = p.id
-- WHERE p.category = 'Gravata'
--   AND p.size = 'Único'
--   AND p.cor = 'Único'
--   AND p.genero = 'Masculino'
--   AND je.estudio_slug IN ('blaze', 'cda')
-- GROUP BY je.estudio_slug
-- ORDER BY je.estudio_slug;
