/**
 * Telecom / job agendado: busca a grade «Todos os jogos» da DonaldBet
 * e chama monitor-lobby-donald (donald_lobby no body).
 *
 * Fonte (F12 → Rede → games/category):
 *   GET https://donald.bet.br/api/games/category/todos-os-jogos?offset=0&limit=48
 *   Página: https://donald.bet.br/cassino/categoria/todos-os-jogos
 *
 * Match: data[].id ↔ Gestão de Estúdios (donald_bet)
 ...
 * Opcional: MONITOR_LOBBY_DONALD_INGEST_SECRET,
 *           DONALD_LOBBY_CATEGORY_URL,
 *           DONALD_LOBBY_CATEGORY,
 *           DONALD_LOBBY_LIMIT,
 *           HTTPS_PROXY / HTTP_PROXY
 *
 * Doc: docs/TELECOM-MONITOR-LOBBY-DONALD.md
 */
...
async function donaldFetch(url, init = {}) {
...
function categoryBaseUrl() {
  const raw =
    process.env.DONALD_LOBBY_CATEGORY_URL?.trim() || CATEGORY_URL_DEFAULT;
  const cat = process.env.DONALD_LOBBY_CATEGORY?.trim() || CATEGORY_DEFAULT;
  if (process.env.DONALD_LOBBY_CATEGORY_URL?.trim()) return raw.replace(/\/$/, "");
  return `${PAGE_ORIGIN}/api/games/category/${cat}`;
}

function perLimit() {
  const n = Number(process.env.DONALD_LOBBY_LIMIT);
...
async function fetchPagina(offset, limit) {
  ...
  const res = await donaldFetch(url);
  if (!res.ok) {
    const body = (await res.text()).slice(0, 300);
    throw new Error(
      `Donald games/category HTTP ${res.status} (offset=${offset}): ${body}`,
    );
  }
  return res.json();
}
...
function ingestHeaders(serviceKey) {
  ...
  const secret = process.env.MONITOR_LOBBY_DONALD_INGEST_SECRET?.trim();
  if (secret) headers["x-monitor-lobby-donald-secret"] = secret;
  return headers;
}
...
async function main() {
  ...
  if (ids.size === 0) {
    console.error(
      "Nenhuma mesa com ID Donald (Gestão de Estúdios → ID Donald Bet).",
    );
    process.exit(1);
  }

  const limit = perLimit();
  console.log(`Buscando lobby Donald (${ids.size} mesas no cadastro)...`);
  ...
  const fnUrl = `${supabaseUrl}/functions/v1/monitor-lobby-donald`;
  const ingestRes = await fetch(fnUrl, {
    ...
    body: JSON.stringify({
      dry_run: dryRun,
      donald_lobby: lobby,
      donald_paginas_lidas: paginasLidas,
    }),
  });
