-- Legado Figurinos — Calça Masculino Preto Staff + Blaze + CDA (self-contained)
...
--
-- Total: 208 peças (27+21+45+30+40+35+10)
-- Escopo: Staff + estúdios blaze e cda (atende_staff = true + N:N)
-- Gênero: Masculino | Cor: Preto | Entrada: 01/01/2026
-- Códigos: prefixo CAL-* (sequência independente de Camisa)

-- ─── Função de lote (com p_atende_staff) ─────────────────────────────────────
...
-- ─── Carga legado Calça Masculino Preto — Staff ──────────────────────────────
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION
      'Estúdios blaze e/ou cda não encontrados ou inativos. SELECT slug, nome FROM estudios_spin WHERE ativo;';
  END IF;

  RAISE NOTICE 'Calça | Staff + blaze/cda | Masculino | Preto | Data: 2026-01-01';
  ...
  LOOP
    ...
    FROM public.rh_figurino_criar_pecas_lote(
      ...
      p_description             := ('Legado — Calça ' || v_row.tamanho || ' Masculino Preto — Staff + Blaze/CDA')::text,
    ...
    RAISE NOTICE 'Calça % Masculino Preto (Staff + Blaze/CDA): % peça(s).', v_row.tamanho, v_row.quantidade;
  END LOOP;

  RAISE NOTICE 'Carga concluída — % peça(s) Calça Staff+Blaze/CDA Preto (CAL-*).', v_total;
END;
$carga$;
...
-- SELECT p.size, count(*) AS qtd
-- FROM public.rh_figurino_pecas p
-- INNER JOIN public.rh_figurino_peca_estudios je ON je.peca_id = p.id
-- WHERE p.category = 'Calça' AND p.atende_staff = true
