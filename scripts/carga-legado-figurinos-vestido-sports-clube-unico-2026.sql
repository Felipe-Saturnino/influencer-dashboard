-- Legado Figurinos — Vestido Feminino Único Sports Club (self-contained)
...
--
-- Total: 62 peças (32 P + 6 M + 14 G + 10 GG)
-- Estúdio: slug sports_clube (Sports Club — network)
-- Gênero: Feminino | Cor: Único | Entrada: 01/01/2026
-- Códigos: prefixo VES-* (sequência compartilhada com demais Vestidos)
...
-- ─── Carga legado Vestido Feminino Único (Sports Club) ───────────────────────
...
BEGIN
  ...
  RAISE NOTICE 'Vestido | Estúdio: sports_clube | Feminino | Único | Data: 2026-01-01';
  ...
  LOOP
    ...
    FROM public.rh_figurino_criar_pecas_lote(
      ...
      p_description             := ('Legado — Vestido ' || v_row.tamanho || ' Feminino Único — Sports Club')::text,
    ...
    RAISE NOTICE 'Vestido % Feminino Único (Sports Club): % peça(s).', v_row.tamanho, v_row.quantidade;
  END LOOP;

  RAISE NOTICE 'Carga concluída — % peça(s) Vestido Sports Club (VES-*).', v_total;
END;
$carga$;
