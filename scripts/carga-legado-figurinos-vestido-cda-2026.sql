-- Legado Figurinos — Vestido Feminino CDA Branco + Único (self-contained)
...
--
-- Total: 140 peças
--   Branco: 20 P + 14 M + 14 G + 8 GG = 56
--   Único:  26 P + 22 M + 17 G + 19 GG = 84
-- Estúdio: slug cda | Gênero: Feminino | Entrada: 01/01/2026
-- Códigos: prefixo VES-* (sequência compartilhada de Vestido)
...
-- ─── Carga legado Vestido Feminino CDA (Branco + Único) ──────────────────────

DO $carga$
DECLARE
  v_est   text[] := ARRAY['cda']::text[];
  v_row   record;
  v_total integer := 0;
  v_n     bigint;
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM public.estudios_spin WHERE slug = 'cda' AND ativo = true
  ) THEN
    RAISE EXCEPTION
      'Estúdio cda não encontrado ou inativo. SELECT slug, nome FROM estudios_spin WHERE ativo;';
  END IF;

  RAISE NOTICE 'Vestido | Estúdio: cda | Feminino | Branco + Único | Data: 2026-01-01';

  FOR v_row IN
    SELECT cor::text AS cor, tamanho::text AS tamanho, quantidade::integer AS quantidade
    FROM (VALUES
      ('Branco', 'P',  20),
      ('Branco', 'M',  14),
      ('Branco', 'G',  14),
      ...
      ('Único',  'G',  17),
      ('Único',  'GG', 19)
    ) AS lotes(cor, tamanho, quantidade)
    ORDER BY cor, array_position(ARRAY['P','M','G','GG']::text[], tamanho)
  LOOP
    ...
    FROM public.rh_figurino_criar_pecas_lote(
      ...
      p_description             := ('Legado — Vestido ' || v_row.tamanho || ' Feminino ' || v_row.cor || ' — CDA')::text,
    ...
    IF v_n <> v_row.quantidade THEN
      RAISE EXCEPTION 'Esperado % peça(s) Vestido % %, cadastradas %',
        v_row.quantidade, v_row.tamanho, v_row.cor, v_n;
    END IF;

    v_total := v_total + v_row.quantidade;
    RAISE NOTICE 'Vestido % Feminino % (CDA): % peça(s).', v_row.tamanho, v_row.cor, v_row.quantidade;
  END LOOP;

  RAISE NOTICE 'Carga concluída — % peça(s) Vestido CDA (VES-*).', v_total;
END;
$carga$;

-- ─── Conferência ─────────────────────────────────────────────────────────────
-- SELECT p.cor, p.size, count(*) AS qtd, min(p.code) AS primeiro, max(p.code) AS ultimo
-- FROM public.rh_figurino_pecas p
-- INNER JOIN public.rh_figurino_peca_estudios je ON je.peca_id = p.id AND je.estudio_slug = 'cda'
-- WHERE p.category = 'Vestido' AND p.genero = 'Feminino' AND p.cor IN ('Branco', 'Único')
-- GROUP BY p.cor, p.size
-- ORDER BY p.cor, array_position(ARRAY['P','M','G','GG']::text[], p.size);
