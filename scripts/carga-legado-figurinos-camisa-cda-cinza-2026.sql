-- Legado Figurinos — Camisa Masculino Cinza CDA (self-contained)
...
--
-- Total: 98 peças (8 PP + 22 P + 28 M + 20 G + 20 GG)
-- Estúdio: slug cda | Gênero: Masculino | Cor: Cinza | Entrada: 01/01/2026
--
-- Complementar aos demais legados Camisa — códigos seguem sequência CAM-*.
...
-- ─── Carga legado Camisa Masculino Cinza (CDA) ───────────────────────────────
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Estúdio cda não encontrado ou inativo. SELECT slug, nome FROM estudios_spin WHERE ativo;';
  END IF;

  RAISE NOTICE 'Estúdio: cda | Gênero: Masculino | Cor: Cinza | Data: 2026-01-01';
  ...
  FOR v_row IN
    ...
    ORDER BY array_position(ARRAY['PP', 'P', 'M', 'G', 'GG'], tamanho)
  LOOP
    ...
    FROM public.rh_figurino_criar_pecas_lote(
      ...
      p_description     := ('Legado — Camisa ' || v_row.tamanho || ' Masculino Cinza — CDA')::text,
    ...
    RAISE NOTICE 'Camisa % Masculino Cinza (CDA): % peça(s).', v_row.tamanho, v_row.quantidade;
  END LOOP;

  RAISE NOTICE 'Carga concluída — % peça(s) CDA Cinza.', v_total;
END;
$carga$;

-- ─── Conferência ─────────────────────────────────────────────────────────────
-- SELECT p.size, p.genero, p.cor, count(*) AS qtd,
--        array_agg(DISTINCT je.estudio_slug) AS estudios
-- FROM public.rh_figurino_pecas p
-- LEFT JOIN public.rh_figurino_peca_estudios je ON je.peca_id = p.id
-- WHERE p.category = 'Camisa' AND p.cor = 'Cinza' AND p.genero = 'Masculino'
-- GROUP BY p.size, p.genero, p.cor
-- ORDER BY array_position(ARRAY['PP','P','M','G','GG'], p.size);
