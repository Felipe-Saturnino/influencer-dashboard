-- =============================================================================
-- COLE NO SUPABASE SQL Editor (projeto produção)
-- Jobs diários manhã via pg_cron — PRINCIPAL (não depende da fila do GitHub Actions)
-- Pré-requisito: vault já tem supabase_project_url + supabase_service_role_key
--   (mesmo setup do cron CS Outlook — scripts/setup-cs-atendimento-outlook-cron.sql)
-- Após colar, valide:
--   SELECT jobid, jobname, schedule, active FROM cron.job WHERE jobname LIKE 'daily-%' ORDER BY schedule;
-- =============================================================================
