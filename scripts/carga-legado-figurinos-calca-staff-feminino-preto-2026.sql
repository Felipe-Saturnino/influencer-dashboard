-- Legado Figurinos — Calça Feminino Preto Staff (self-contained)
...
--
-- Total: 43 peças (2 P + 2 M + 6 G + 2 GG + 2×36 + 11×38 + 2×40 + 8×42 + 2×44 + 4×46 + 2×50)
...
-- ─── Carga legado Calça Feminino Preto — Staff ───────────────────────────────
...
BEGIN
  ...
  RAISE NOTICE 'Calça | Staff | Feminino | Preto | Data: 2026-01-01';
  ...
  FOR v_row IN
    ...
    ORDER BY array_position(
      ARRAY['P','M','G','GG','XG','PP','34','36','38','40','42','44','46','48','50','52','Único']::text[],
      tamanho
    )
  LOOP
    ...
    FROM public.rh_figurino_criar_pecas_lote(
      ...
      p_description             := ('Legado — Calça ' || v_row.tamanho || ' Feminino Preto — Staff')::text,
    ...
    RAISE NOTICE 'Calça % Feminino Preto (Staff): % peça(s).', v_row.tamanho, v_row.quantidade;
  END LOOP;

  RAISE NOTICE 'Carga concluída — % peça(s) Calça Staff Feminino Preto (CAL-*).', v_total;
END;
$carga$;
...
-- GROUP BY p.size
-- ORDER BY array_position(
--   ARRAY['P','M','G','GG','36','38','40','42','44','46','50']::text[], p.size
-- );
