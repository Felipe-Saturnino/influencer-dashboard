-- Legado Figurinos — Colete Masculino Único Blaze (self-contained)
...
--
-- Total: 20 peças (5 P + 7 M + 5 G + 3 GG)
-- Estúdio: slug blaze | Gênero: Masculino | Cor: Único | Entrada: 01/01/2026
-- Códigos: prefixo COL-* (Colete)
...
-- ─── Carga legado Colete Masculino Único (Blaze) ─────────────────────────────
...
BEGIN
  ...
  RAISE NOTICE 'Colete | Estúdio: blaze | Masculino | Único | Data: 2026-01-01';
  ...
  LOOP
    ...
    IF v_n <> v_row.quantidade THEN
      RAISE EXCEPTION 'Esperado % peça(s) Colete %, cadastradas %', v_row.quantidade, v_row.tamanho, v_n;
    END IF;

    v_total := v_total + v_row.quantidade;
    RAISE NOTICE 'Colete % Masculino Único (Blaze): % peça(s).', v_row.tamanho, v_row.quantidade;
  END LOOP;

  RAISE NOTICE 'Carga concluída — % peça(s) Colete Blaze (COL-*).', v_total;
END;
$carga$;
...
-- WHERE p.category = 'Colete' AND p.cor = 'Único' AND p.genero = 'Masculino'
-- GROUP BY p.size
-- ORDER BY array_position(ARRAY['P','M','G','GG']::text[], p.size);
