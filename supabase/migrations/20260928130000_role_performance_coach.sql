-- Perfil Performance Coach: paridade com Figurino (staff interno, role_permissions, sem user_scopes operadora).
...
SELECT 'performance_coach', page_key, can_view, can_criar, can_editar, can_excluir
...
COMMENT ON FUNCTION public._role_sem_escopo_app() IS
  'Executivo, Investidor, Prestador e staff Spin (incl. Performance Coach): sem escopo operadora/influencer na app — só role_permissions.';
...
COMMENT ON FUNCTION public._staff_spin_page_perm(text, text) IS
  'Shift Leader / Service Manager / Figurino / Comunicação / Performance Coach / RH: ação efetiva só conforme role_permissions (aba Permissões), sem user_scopes.';
