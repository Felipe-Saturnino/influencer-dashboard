-- ─── Assinatura válida em PostgreSQL: parâmetro obrigatório antes de DEFAULT ─────
-- (p_utm_source text, p_influencer_id uuid DEFAULT NULL). PostgREST/Supabase mapeia
-- os args do JSON pelo nome, não pela ordem alfabética.

DROP FUNCTION IF EXISTS public.registrar_utm_alias_tracking_casa_apostas(text);
DROP FUNCTION IF EXISTS public.registrar_utm_alias_tracking_casa_apostas(text, uuid);
DROP FUNCTION IF EXISTS public.registrar_utm_alias_tracking_casa_apostas(uuid, text);
