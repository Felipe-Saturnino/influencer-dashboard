-- Histórico de justificativa médica: persiste atendente/data do atendimento em Solicitações RH.
...
BEGIN
  ...
  SELECT s.atestado_inicio, s.atestado_fim, coalesce(s.atendido_em, s.updated_at), p.name
  INTO v_inicio, v_fim, v_atendido_em, v_atendido_nome
...
-- Backfill atendimento em justificativas médicas já aprovadas/rejeitadas.
DO $$
DECLARE
  r RECORD;
BEGIN
