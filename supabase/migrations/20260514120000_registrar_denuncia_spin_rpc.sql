-- Registro público via RPC (SECURITY DEFINER): evita 42501 quando o JWT / RLS
-- no remoto não corresponde ao esperado (ex.: políticas não aplicadas ao role efetivo).
...
COMMENT ON FUNCTION public.registrar_denuncia_spin(boolean, text, text, text, text[], text, text) IS
  'Canal de denúncias Spin — insert público (bypass RLS); validação mínima de campos.';
