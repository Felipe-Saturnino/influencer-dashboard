-- Fix: espelho CT Feedback → Solicitações via RPC (não depende só do trigger)
-- + backfill dos feedbacks já criados sem linha em rh_solicitacoes.
-- Idempotente — pode rodar mesmo se 20261209180000 ainda não tiver sido aplicada.

BEGIN;

-- ─── Colunas / checks mínimos (idempotente) ─────────────────────────────────
...
-- ─── Helper: upsert solicitação a partir de uma linha CT ─────────────────────
...
REVOKE ALL ON FUNCTION public._rh_solicitacoes_upsert_from_ct_feedback(uuid, uuid, text, text, text, text) FROM PUBLIC;

-- ─── Trigger CT → Solicitações (usa helper; idempotente) ─────────────────────
...
-- ─── RPC: criar feedback CT + espelho Solicitações ───────────────────────────
...
BEGIN
  ...
  -- Espelho explícito (além do trigger) — garante linha mesmo se trigger falhar
  PERFORM public._rh_solicitacoes_upsert_from_ct_feedback(
    v_id,
    p_prestador_id,
    p_recomendacao,
...
COMMENT ON FUNCTION public.escala_ct_feedback_criar(date, uuid, text, text, text) IS
  'Controle de Turno: cria feedback e espelha em rh_solicitacoes (origem controle_turno).';

-- ─── RPC: backfill feedbacks CT sem solicitação ──────────────────────────────
...
COMMENT ON FUNCTION public.rh_solicitacoes_backfill_feedback_ct() IS
  'Cria em Solicitações as linhas de feedback do CT que ainda não têm espelho.';

-- ─── Solicitações → CT (Atender Aprovado/Rejeitado) ──────────────────────────
