-- Atendimento CS: filtro Staff passa a listar prestadores do time Service Manager
-- (time Customer Service descontinuado no Organograma).
...
AS $$
  ...
  FROM (
    ...
    INNER JOIN public.rh_org_times t
      ...
      AND t.status = 'ativo'
      AND lower(regexp_replace(btrim(t.nome), '\s+', ' ', 'g')) = 'service manager'
...
COMMENT ON FUNCTION public.cs_atendimento_atendentes_listar() IS
  'Atendimento CS: prestadores ativos/indisponíveis do time Service Manager (organograma), mapeados a profiles.id quando houver login. Requer cs_atendimento.can_view ou admin.';
