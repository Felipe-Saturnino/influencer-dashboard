-- Escala do Turno: ponto (check-in/out) só entra no turno cuja janela de horário
-- corresponde ao check-in — evita ponto da Manhã pintar Presente na Noite do mesmo dia.
  ...
  ponto AS (
    SELECT
      ...
      min(r.created_at) FILTER (
        WHERE r.tipo = 'check_in'
          AND (
            CASE v_turno
              WHEN 'manha' THEN
                EXTRACT(HOUR FROM (r.created_at AT TIME ZONE 'America/Sao_Paulo')) BETWEEN 5 AND 14
              WHEN 'tarde' THEN
                EXTRACT(HOUR FROM (r.created_at AT TIME ZONE 'America/Sao_Paulo')) BETWEEN 12 AND 21
              ELSE
                EXTRACT(HOUR FROM (r.created_at AT TIME ZONE 'America/Sao_Paulo')) >= 17
                OR EXTRACT(HOUR FROM (r.created_at AT TIME ZONE 'America/Sao_Paulo')) < 5
            END
          )
      ) AS check_in_at,
  ...
  linhas AS (
    SELECT
      ...
      CASE
        ...
        WHEN p.check_in_at IS NOT NULL
             AND p.check_out_at IS NOT NULL
             AND p.check_out_at >= p.check_in_at
...
COMMENT ON FUNCTION public.escala_controle_turno_presenca_dia(date, text) IS
  'Controle de Turno → Escala do Turno: escalados + ponto (filtrado por janela do turno) + overlay. registrado = aprovado.';
