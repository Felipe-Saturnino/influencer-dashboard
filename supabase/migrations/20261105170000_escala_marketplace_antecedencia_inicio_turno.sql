-- Marketplace: antecedência de 24h passa a ser pelo início do turno (cliente).
-- O banco só exige dia ≥ hoje (America/Sao_Paulo); horários de operadora/escala
-- vivem no cliente (`turnoRespeitaAntecedencia24h`).
...
BEGIN
  ...
  -- Dia calendário ≥ hoje; a regra de ≥24h até o início do turno é do cliente.
...
COMMENT ON FUNCTION public.escala_marketplace_oferta_criar(text, date, text, text, text) IS
  'Marketplace: cria oferta. Dia ≥ hoje (SP); antecedência ≥24h até o início do turno é validada no cliente.';
