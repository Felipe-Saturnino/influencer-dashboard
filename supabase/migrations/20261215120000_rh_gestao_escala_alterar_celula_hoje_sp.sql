-- Alterar Escala: "hoje" no fuso America/Sao_Paulo (não current_date UTC).
-- Supabase usa UTC: após ~21h BRT, current_date já é o dia seguinte e rejeitava
-- o dia civil de hoje no Brasil com error `dia_passado` («Só é possível alterar dias a partir de hoje.»),
-- enquanto a UI (relógio local/SP) ainda liberava a célula.
...
DECLARE
  ...
  v_hoje_sp date := (timezone('America/Sao_Paulo', now()))::date;
BEGIN
  ...
  -- Dia civil em São Paulo (paridade com Marketplace e UI).
  IF p_dia_iso < v_hoje_sp THEN
...
COMMENT ON FUNCTION public.rh_gestao_escala_grade_alterar_celula(date, text, uuid, date, text, text) IS
  'Altera célula da grade aprovada. «Hoje» = America/Sao_Paulo. Observação obrigatória. Log + histórico.';
