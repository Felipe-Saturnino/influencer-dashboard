-- Links e Materiais: emissão deve persistir mapeado + influencer_id e espelhar utm_source no perfil.
-- Repara aliases rebaixados para pendente pelo sync CDA (mapeado_por preenchido) e garante can_editar.

BEGIN;

-- Permissão de emissão (idempotente — migration 202604030 pode não ter corrido em todos os ambientes)
UPDATE public.role_permissions
SET can_editar = 'proprios'
WHERE page_key = 'links_materiais'
  AND role IN ('influencer', 'afiliado')
  AND can_editar IS NULL;

-- Repara links emitidos via Links e Materiais que ficaram com status pendente após sync de órfãos
UPDATE public.utm_aliases ua
SET
  status = 'mapeado',
  influencer_id = COALESCE(ua.influencer_id, ua.mapeado_por),
  atualizado_em = now()
WHERE ua.status = 'pendente'
  AND ua.campanha_id IS NULL
  AND ua.mapeado_por IS NOT NULL
  AND ua.mapeado_em IS NOT NULL
  AND (ua.operadora_slug = 'casa_apostas' OR ua.operadora_slug IS NULL)
  AND EXISTS (
    SELECT 1 FROM public.influencer_perfil ip
    WHERE ip.id = COALESCE(ua.influencer_id, ua.mapeado_por)
  );

UPDATE public.influencer_perfil ip
SET utm_source = ua.utm_source
FROM public.utm_aliases ua
WHERE ua.influencer_id = ip.id
  AND ua.status = 'mapeado'
  AND ua.campanha_id IS NULL
  AND (ua.operadora_slug = 'casa_apostas' OR ua.operadora_slug IS NULL)
  AND (ip.utm_source IS DISTINCT FROM ua.utm_source);
...
COMMENT ON FUNCTION public.registrar_utm_alias_tracking_casa_apostas(text, uuid) IS
  'Emite link CDA (Links e Materiais): grava utm_aliases mapeado, espelha utm_source em influencer_perfil e aplica métricas.';
