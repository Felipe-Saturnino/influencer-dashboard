-- Escala — Rotação (page_key escala_rotacao).
-- Grade GP × mesa × slot; pool a partir da escala aprovada + Staff 4×2 + mesas do estúdio.
...
COMMENT ON FUNCTION public._escala_rotacao_perm(text) IS
  'Permissão Escala → Rotação (view|create|edit|delete).';

-- ─── Tabelas ─────────────────────────────────────────────────────────────────

CREATE TABLE public.escala_rotacao (
...
CREATE UNIQUE INDEX escala_rotacao_publicada_uk
  ON public.escala_rotacao (dia, turno, estudio_slug)
  WHERE status = 'publicada';

CREATE INDEX escala_rotacao_dia_idx ON public.escala_rotacao (dia DESC);
CREATE INDEX escala_rotacao_estudio_idx ON public.escala_rotacao (estudio_slug);

COMMENT ON TABLE public.escala_rotacao IS
  'Cabeçalho da rotação de Game Presenters (dia + turno + estúdio).';

CREATE TABLE public.escala_rotacao_celula (
  id              uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX escala_rotacao_celula_rot_idx ON public.escala_rotacao_celula (rotacao_id);
CREATE INDEX escala_rotacao_celula_func_idx ON public.escala_rotacao_celula (funcionario_id);

COMMENT ON TABLE public.escala_rotacao_celula IS
  'Célula da rotação: GP × slot → mesa_identificacao | B | F.';

COMMENT ON COLUMN public.escala_rotacao_celula.valor IS
  'mesa_identificacao, B (break) ou F (falta).';
...
-- ─── Contexto do dia (pool + mesas + horários) ────────────────────────────────
...
BEGIN
  ...
  IF v_inicio IS NULL THEN
    -- fallback operacional alinhado ao Excel CDA se o estúdio ainda não tiver horário
  ...
  SELECT COALESCE(jsonb_agg(
    ...
    ORDER BY m.nome_mesa, m.mesa_identificacao
  ), '[]'::jsonb)
  INTO v_mesas
  FROM public.mesas_spin_cadastro m
  WHERE m.estudio_slug = v_estudio
    AND btrim(COALESCE(m.mesa_identificacao, '')) <> '';
  ...
  IF v_aprovada THEN
    ...
    WHERE f.status IN ('ativo', 'indisponivel')
      ...
      AND lower(regexp_replace(COALESCE(f.escala, ''), '\s+', '', 'g')) = '4x2'
      AND (
...
COMMENT ON FUNCTION public.escala_rotacao_contexto_dia(date, text, text) IS
  'Pool de GPs 4×2 escalados (grade aprovada) + mesas + horário 8h do estúdio para a Rotação.';

-- ─── Publicar (substitui publicada anterior) ─────────────────────────────────
...
COMMENT ON FUNCTION public.escala_rotacao_publicar(jsonb) IS
  'Publica rotação do dia/turno/estúdio (arquiva a anterior publicada).';
...
COMMENT ON FUNCTION public._estudios_spin_leitura_perm(text) IS
  'SELECT em estudios_spin: Gestão de Mesas ou páginas operacionais (incl. Relatório de Turno e Rotação).';
