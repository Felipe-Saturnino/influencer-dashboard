...
-- na página «Dados de Cadastro» (sem permissão rh_funcionarios edit).
...
BEGIN
  ...
  -- Quem edita via Gestão de Prestadores (permissão global) já regista histórico noutros fluxos
  IF public._rh_funcionario_perm('edit') THEN
    RETURN NEW;
  END IF;

  -- Apenas o próprio prestador (identidade antes do UPDATE, alinhado às políticas self)
  IF NOT EXISTS (
    SELECT 1
...
COMMENT ON FUNCTION public.rh_funcionarios_log_historico_dados_cadastro_self() IS
  'Regista em rh_funcionario_historico alterações ao cadastro feitas pelo próprio colaborador (Dados de Cadastro).';
