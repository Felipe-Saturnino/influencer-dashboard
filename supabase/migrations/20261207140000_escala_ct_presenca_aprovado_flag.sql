-- Escala do Turno: coluna aprovado — só a ação Aprovar marca Sim.
-- Outros tipos (falta, saída antecipada, etc.) não alteram aprovado.
...
COMMENT ON COLUMN public.escala_ct_presenca_registro.aprovado IS
  'True somente após a ação Aprovar na Escala do Turno. Demais registros não alteram este flag.';

-- Legado: registros já tipados como aprovar passam a Sim
  ...
  linhas AS (
    SELECT
      ...
      CASE
        ...
        ELSE 'pendente'
      END AS status,
      -- Aprovado = Sim somente após a ação Aprovar (coluna aprovado)
...
COMMENT ON FUNCTION public.escala_controle_turno_presenca_dia(date, text) IS
  'Controle de Turno → Escala do Turno: escalados + ponto + overlay. Campo registrado = aprovado (só ação Aprovar).';
