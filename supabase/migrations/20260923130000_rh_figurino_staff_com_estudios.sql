-- Figurinos: Staff pode combinar com estúdio(s) específico(s); só «Todos Estúdios» fica exclusivo.

BEGIN;

COMMENT ON COLUMN public.rh_figurino_pecas.atende_staff IS
  'Quando true, peça do acervo Staff. Pode combinar com N:N em rh_figurino_peca_estudios; exclusivo apenas contra atende_todos_estudios.';

DROP FUNCTION IF EXISTS public.rh_figurino_criar_peca(text[], text, text, text, text, date, text, text, boolean, boolean);
...
BEGIN
  ...
  IF coalesce(p_atende_todos_estudios, false) THEN
    ...
    END IF;
  ELSIF coalesce(p_atende_staff, false) AND NOT v_tem_slugs THEN
    IF NOT public._rh_figurino_auth_can_criar_peca() THEN
      RAISE EXCEPTION 'rh_figurino_forbidden' USING ERRCODE = 'P0001';
    END IF;
  END IF;
  ...
  VALUES (
    ...
    CASE
      WHEN coalesce(p_atende_staff, false) AND v_tem_slugs THEN 'Cadastro — staff + estúdios'
...
COMMENT ON FUNCTION public.rh_figurino_criar_peca(text[], text, text, text, text, date, text, text, boolean, boolean) IS
  'Cadastra peça. Todos Estúdios é exclusivo; Staff pode combinar com p_estudio_slugs.';
