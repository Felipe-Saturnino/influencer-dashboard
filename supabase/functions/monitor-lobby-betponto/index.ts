...
/**
 * Edge Function: monitor-lobby-betponto
 * Lê a grade «Todos os jogos» da BetPontoBet, grava posição das mesas Spin
 * e concorrentes do mesmo tipo à frente.
 *
 * Fontes de ID (união, dedupe por mesa Spin):
 * 1. `mesas_spin_operadora_identificacao` onde operadora_slug = betponto_bet
 * 2. Legado: `mesas_spin_cadastro.operadora_slug = betponto_bet` + mesa_identificacao_operadora
 *
 * Match: `data[].id` de games/category ↔ ID na Gestão de Estúdios.
 * Fonte produto: https://betpontobet.bet.br/cassino/categoria/todos-os-jogos
 ...
 *
 * Chamada: POST {} ou { dry_run?: boolean, betponto_lobby?: LobbyGame[] }
 * Segurança: MONITOR_LOBBY_BETPONTO_INGEST_SECRET (header x-monitor-lobby-betponto-secret)
 *   ou Authorization: Bearer <SUPABASE_SERVICE_ROLE_KEY>
 *
 * Deploy: supabase functions deploy monitor-lobby-betponto
 */

const OPERADORA_SLUG = "betponto_bet";
const INTEGRACAO_SLUG = "lobby_betponto";
const CATEGORY_URL =
...
function betpontoFetchHeaders(): Record<string, string> {
  return {
...
interface MonitorBody {
  ...
  betponto_lobby?: LobbyGame[];
  betponto_paginas_lidas?: number;
}
...
function mergeMesasMonitorBetponto(
...
async function carregarMesasMonitorBetponto(
  ...
  return {
    mesas: mergeMesasMonitorBetponto(
...
function corsHeaders(req: Request): Record<string, string> {
  ...
  return {
    ...
    "Access-Control-Allow-Headers":
      "authorization, x-client-info, apikey, content-type, x-monitor-lobby-betponto-secret",
...
function autorizado(req: Request): boolean {
  const secret = Deno.env.get("MONITOR_LOBBY_BETPONTO_INGEST_SECRET")?.trim();
  if (!secret) return true;
  const h =
    req.headers.get("x-monitor-lobby-betponto-secret") ??
    req.headers.get("X-Monitor-Lobby-Betponto-Secret");
...
async function fetchPagina(offset: number): Promise<ListResponse> {
  ...
  const res = await fetch(url, { headers: betpontoFetchHeaders() });
  if (!res.ok) {
    throw new Error(
      `BetPontoBet games/category HTTP ${res.status} (offset=${offset}). Se bloquear datacenter, use o script Telecom com betponto_lobby.`,
    );
  }
...
async function gravarSyncLogLobby(
  ...
  try {
    ...
    console.error("[monitor-lobby-betponto] Falha ao gravar sync_logs:", e);
  }
}
...
serve(async (req) => {
  ...
  const { mesas: mesasList, erro: mesasLoadErr } =
    await carregarMesasMonitorBetponto(supabase);
  if (mesasLoadErr) {
  ...
  if (mesasList.length === 0) {
    return json({
      ...
      status: "erro_config",
      erro:
        `Nenhuma mesa BetPontoBet com ID: preencha ID BetPontoBet em Gestão de Estúdios (operadora_slug=${OPERADORA_SLUG})`,
    }, req, 200);
  }
  ...
  if (Array.isArray(body.betponto_lobby) && body.betponto_lobby.length > 0) {
    lobby = body.betponto_lobby.map((g, i) => ({
      ...g,
      game_id: String(g.game_id),
      posicao: typeof g.posicao === "number" ? g.posicao : i + 1,
    }));
    paginasLidas = body.betponto_paginas_lidas ?? 1;
