...
/**
 * Edge Function: monitor-lobby-goldebet
 * Lê a grade Cassino Ao Vivo (Live) da Goldebet, grava posição das mesas Spin
 * e concorrentes do mesmo tipo à frente.
 *
 * Fontes de ID (união, dedupe por mesa Spin):
 * 1. `mesas_spin_operadora_identificacao` onde operadora_slug = goldebet
 * 2. Legado: `mesas_spin_cadastro.operadora_slug = goldebet` + mesa_identificacao_operadora
 *
 * Match: `String(data[].id)` de v2/casino-games ↔ ID na Gestão de Estúdios (IDs numéricos).
 * Fonte produto: https://goldebet.bet.br/casino/live
 *   GET /v2/casino-games?page=&casino_game_category_ids[]=2&order=clicks
 *   Resposta: { data, links, meta } — meta.last_page / meta.per_page (~20) / meta.total
 * Concorrentes: mesmo tipo de jogo cujo id NÃO está na lista Spin.
 *
 * Chamada: POST {} ou { dry_run?: boolean, goldebet_lobby?: LobbyGame[] }
 * Segurança: MONITOR_LOBBY_GOLDEBET_INGEST_SECRET (header x-monitor-lobby-goldebet-secret)
 *   ou Authorization: Bearer <SUPABASE_SERVICE_ROLE_KEY>
 *
 * Deploy: supabase functions deploy monitor-lobby-goldebet
 */

const OPERADORA_SLUG = "goldebet";
const INTEGRACAO_SLUG = "lobby_goldebet";
const GAMES_URL = "https://goldebet.bet.br/v2/casino-games";
const CATEGORY_ID = "2";
const ORDER = "clicks";
const PER_PAGE = 20;
const MAX_PAGES = 200;
...
function goldebetFetchHeaders(): Record<string, string> {
  return {
...
interface MonitorBody {
  ...
  goldebet_lobby?: LobbyGame[];
  goldebet_paginas_lidas?: number;
}
...
function mergeMesasMonitorGoldebet(
...
async function carregarMesasMonitorGoldebet(
  ...
  return {
    mesas: mergeMesasMonitorGoldebet(
...
function corsHeaders(req: Request): Record<string, string> {
  ...
  return {
    ...
    "Access-Control-Allow-Headers":
      "authorization, x-client-info, apikey, content-type, x-monitor-lobby-goldebet-secret",
...
function autorizado(req: Request): boolean {
  const secret = Deno.env.get("MONITOR_LOBBY_GOLDEBET_INGEST_SECRET")?.trim();
  if (!secret) return true;
  const h =
    req.headers.get("x-monitor-lobby-goldebet-secret") ??
    req.headers.get("X-Monitor-Lobby-Goldebet-Secret");
...
async function fetchPagina(page: number): Promise<ListResponse> {
  ...
  params.append("casino_game_category_ids[]", CATEGORY_ID);
  params.set("order", ORDER);
  const url = `${GAMES_URL}?${params.toString()}`;
  const res = await fetch(url, { headers: goldebetFetchHeaders() });
  if (!res.ok) {
    throw new Error(
      `Goldebet v2/casino-games HTTP ${res.status} (page=${page}). Se bloquear datacenter, use o script Telecom com goldebet_lobby.`,
    );
  }
...
async function escanearLobby(
  ...
  while (page <= lastPage && page <= MAX_PAGES) {
    ...
    if (page === 1) {
      ...
      perPage = Math.max(1, Number(data.per_page) || PER_PAGE);
    }
...
async function gravarSyncLogLobby(
  ...
  try {
    ...
    console.error("[monitor-lobby-goldebet] Falha ao gravar sync_logs:", e);
  }
}
...
serve(async (req) => {
  ...
  const { mesas: mesasList, erro: mesasLoadErr } =
    await carregarMesasMonitorGoldebet(supabase);
  if (mesasLoadErr) {
  ...
  if (mesasList.length === 0) {
    return json({
      ...
      status: "erro_config",
      erro:
        `Nenhuma mesa Goldebet com ID: preencha ID Goldebet em Gestão de Estúdios (operadora_slug=${OPERADORA_SLUG})`,
    }, req, 200);
  }
  ...
  if (Array.isArray(body.goldebet_lobby) && body.goldebet_lobby.length > 0) {
    lobby = body.goldebet_lobby.map((g, i) => ({
      ...g,
      game_id: String(g.game_id),
      posicao: typeof g.posicao === "number" ? g.posicao : i + 1,
    }));
    paginasLidas = body.goldebet_paginas_lidas ?? 1;
