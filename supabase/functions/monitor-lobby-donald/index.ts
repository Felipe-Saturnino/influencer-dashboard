...
/**
 * Edge Function: monitor-lobby-donald
 * Lê a grade «Todos os jogos» da DonaldBet, grava posição das mesas Spin
 * e concorrentes do mesmo tipo à frente.
 *
 * Fontes de ID (união, dedupe por mesa Spin):
 * 1. `mesas_spin_operadora_identificacao` onde operadora_slug = donald_bet
 * 2. Legado: `mesas_spin_cadastro.operadora_slug = donald_bet` + mesa_identificacao_operadora
 *
 * Match: `data[].id` de games/category ↔ ID na Gestão de Estúdios.
 * Fonte produto: https://donald.bet.br/cassino/categoria/todos-os-jogos
 *   GET /api/games/category/todos-os-jogos?offset=&limit=48
 * Concorrentes: mesmo tipo de jogo cujo id NÃO está na lista Spin.
 *
 * Nota (validação 2026-09): mesas GG Labs podem existir em
 * `/api/games/provider/goodgame` e ainda não aparecer na grade Todos os jogos.
 *
 * Chamada: POST {} ou { dry_run?: boolean, donald_lobby?: LobbyGame[] }
 * Segurança: MONITOR_LOBBY_DONALD_INGEST_SECRET (header x-monitor-lobby-donald-secret)
 *   ou Authorization: Bearer <SUPABASE_SERVICE_ROLE_KEY>
 *
 * Deploy: supabase functions deploy monitor-lobby-donald
 */

const OPERADORA_SLUG = "donald_bet";
const INTEGRACAO_SLUG = "lobby_donald";
const CATEGORY_URL =
  "https://donald.bet.br/api/games/category/todos-os-jogos";
const LIMIT = 48;
const MAX_PAGES = 200;
const PAGE_ORIGIN = "https://donald.bet.br";
const PAGE_REFERER = `${PAGE_ORIGIN}/cassino/categoria/todos-os-jogos`;

function donaldFetchHeaders(): Record<string, string> {
  return {
...
interface MonitorBody {
  ...
  donald_lobby?: LobbyGame[];
  donald_paginas_lidas?: number;
}
...
function mergeMesasMonitorDonald(
...
async function carregarMesasMonitorDonald(
...
function corsHeaders(req: Request): Record<string, string> {
  ...
  return {
    ...
    "Access-Control-Allow-Headers":
      "authorization, x-client-info, apikey, content-type, x-monitor-lobby-donald-secret",
...
function autorizado(req: Request): boolean {
  const secret = Deno.env.get("MONITOR_LOBBY_DONALD_INGEST_SECRET")?.trim();
  if (!secret) return true;
  const h =
    req.headers.get("x-monitor-lobby-donald-secret") ??
    req.headers.get("X-Monitor-Lobby-Donald-Secret");
...
async function fetchPagina(offset: number): Promise<ListResponse> {
  const url = `${CATEGORY_URL}?offset=${offset}&limit=${LIMIT}`;
  const res = await fetch(url, { headers: donaldFetchHeaders() });
  if (!res.ok) {
    throw new Error(
      `Donald games/category HTTP ${res.status} (offset=${offset}). Se bloquear datacenter, use o script Telecom com donald_lobby.`,
    );
  }
...
async function escanearLobby(
  ...
  return { lobby, posicoes, paginasLidas };
}
...
async function gravarSyncLogLobby(
  ...
  try {
    ...
    console.error("[monitor-lobby-donald] Falha ao gravar sync_logs:", e);
  }
}
...
serve(async (req) => {
  ...
  const { mesas: mesasList, erro: mesasLoadErr } =
    await carregarMesasMonitorDonald(supabase);
  if (mesasLoadErr) {
  ...
  if (mesasList.length === 0) {
    return json({
      ...
      status: "erro_config",
      erro:
        `Nenhuma mesa Donald com ID: preencha ID Donald Bet em Gestão de Estúdios (operadora_slug=${OPERADORA_SLUG})`,
    }, req, 200);
  }
  ...
  if (Array.isArray(body.donald_lobby) && body.donald_lobby.length > 0) {
    lobby = body.donald_lobby.map((g, i) => ({
      ...g,
      game_id: String(g.game_id),
      posicao: typeof g.posicao === "number" ? g.posicao : i + 1,
    }));
    paginasLidas = body.donald_paginas_lidas ?? 1;
