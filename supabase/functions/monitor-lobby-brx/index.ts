...
/**
 * Edge Function: monitor-lobby-brx
 * Lê a grade «Todos os jogos» (/games/category/all) da Brx.bet, grava posição das mesas Spin
 * e concorrentes do mesmo tipo à frente.
 *
 * Fontes de ID (união, dedupe por mesa Spin):
 * 1. `mesas_spin_operadora_identificacao` onde operadora_slug = brx_bet
 * 2. Legado: `mesas_spin_cadastro.operadora_slug = brx_bet` + mesa_identificacao_operadora
 *
 * Match: `data[].id` de casino-games/filter ↔ ID na Gestão de Estúdios.
 * Fonte produto: https://brx.bet.br/games/category/all
 *   GET /api/casino-games/filter?sort=&per_page=24&page=&term=
 * Concorrentes: mesmo tipo de jogo cujo id NÃO está na lista Spin.
 *
 * Chamada: POST {} ou { dry_run?: boolean, brx_lobby?: LobbyGame[] }
 * Segurança: MONITOR_LOBBY_BRX_INGEST_SECRET (header x-monitor-lobby-brx-secret)
 *   ou Authorization: Bearer <SUPABASE_SERVICE_ROLE_KEY>
 *
 * Deploy: supabase functions deploy monitor-lobby-brx
 */

const OPERADORA_SLUG = "brx_bet";
const INTEGRACAO_SLUG = "lobby_brx";
const FILTER_URL = "https://brx.bet.br/api/casino-games/filter";
const PER_PAGE = 24;
const MAX_PAGES = 300;
const PAGE_ORIGIN = "https://brx.bet.br";
const PAGE_REFERER = `${PAGE_ORIGIN}/games/category/all`;

function brxFetchHeaders(): Record<string, string> {
  return {
...
async function carregarMesasMonitorBrx(
...
function corsHeaders(req: Request): Record<string, string> {
  ...
  return {
    ...
    "Access-Control-Allow-Headers":
      "authorization, x-client-info, apikey, content-type, x-monitor-lobby-brx-secret",
...
function autorizado(req: Request): boolean {
  const secret = Deno.env.get("MONITOR_LOBBY_BRX_INGEST_SECRET")?.trim();
  if (!secret) return true;
  const h =
    req.headers.get("x-monitor-lobby-brx-secret") ??
    req.headers.get("X-Monitor-Lobby-Brx-Secret");
...
async function fetchPagina(page: number): Promise<ListResponse> {
  ...
  const res = await fetch(url, { headers: brxFetchHeaders() });
  if (!res.ok) {
    throw new Error(
      `BRX casino-games/filter HTTP ${res.status} (page=${page}). Se bloquear datacenter, use o script Telecom com brx_lobby.`,
    );
  }
...
async function gravarSyncLogLobby(
  ...
  try {
    ...
    console.error("[monitor-lobby-brx] Falha ao gravar sync_logs:", e);
  }
}
...
serve(async (req) => {
  ...
  const { mesas: mesasList, erro: mesasLoadErr } =
    await carregarMesasMonitorBrx(supabase);
  if (mesasLoadErr) {
  ...
  if (mesasList.length === 0) {
    return json({
      ...
      status: "erro_config",
      erro:
        `Nenhuma mesa BRX com ID: preencha ID BRX Bet em Gestão de Estúdios (operadora_slug=${OPERADORA_SLUG})`,
    }, req, 200);
  }
  ...
  if (Array.isArray(body.brx_lobby) && body.brx_lobby.length > 0) {
    lobby = body.brx_lobby.map((g, i) => ({
      ...g,
      game_id: String(g.game_id),
      posicao: typeof g.posicao === "number" ? g.posicao : i + 1,
    }));
    paginasLidas = body.brx_paginas_lidas ?? 1;
