...
/**
 * Edge Function: monitor-lobby-rico
 * Lê a grade «Todos os jogos» (/games/category/all) da Rico.bet, grava posição das mesas Spin
 * e concorrentes do mesmo tipo à frente.
 *
 * Fontes de ID (união, dedupe por mesa Spin):
 * 1. `mesas_spin_operadora_identificacao` onde operadora_slug = rico_bet
 * 2. Legado: `mesas_spin_cadastro.operadora_slug = rico_bet` + mesa_identificacao_operadora
 *
 * Match: `data[].id` de casino-games/filter ↔ ID na Gestão de Estúdios.
 * Fonte produto: https://rico.bet.br/games/category/all
 *   GET /api/casino-games/filter?sort=&per_page=24&page=&term=
 * Concorrentes: mesmo tipo de jogo cujo id NÃO está na lista Spin.
 *
 * Chamada: POST {} ou { dry_run?: boolean, rico_lobby?: LobbyGame[] }
 * Segurança: MONITOR_LOBBY_RICO_INGEST_SECRET (header x-monitor-lobby-rico-secret)
 *   ou Authorization: Bearer <SUPABASE_SERVICE_ROLE_KEY>
 *
 * Deploy: supabase functions deploy monitor-lobby-rico
 */

const OPERADORA_SLUG = "rico_bet";
const INTEGRACAO_SLUG = "lobby_rico";
const FILTER_URL = "https://rico.bet.br/api/casino-games/filter";
const PER_PAGE = 24;
const MAX_PAGES = 300;
const PAGE_ORIGIN = "https://rico.bet.br";
const PAGE_REFERER = `${PAGE_ORIGIN}/games/category/all`;

function ricoFetchHeaders(): Record<string, string> {
  return {
...
interface MonitorBody {
  ...
  rico_lobby?: LobbyGame[];
  rico_paginas_lidas?: number;
}
...
function mergeMesasMonitorRico(
...
async function carregarMesasMonitorRico(
...
function corsHeaders(req: Request): Record<string, string> {
  ...
  return {
    ...
    "Access-Control-Allow-Headers":
      "authorization, x-client-info, apikey, content-type, x-monitor-lobby-rico-secret",
...
function autorizado(req: Request): boolean {
  const secret = Deno.env.get("MONITOR_LOBBY_RICO_INGEST_SECRET")?.trim();
  if (!secret) return true;
  const h =
    req.headers.get("x-monitor-lobby-rico-secret") ??
    req.headers.get("X-Monitor-Lobby-Rico-Secret");
...
async function fetchPagina(page: number): Promise<ListResponse> {
  ...
  const res = await fetch(url, { headers: ricoFetchHeaders() });
  if (!res.ok) {
    throw new Error(
      `Rico casino-games/filter HTTP ${res.status} (page=${page}). Se bloquear datacenter, use o script Telecom com rico_lobby.`,
    );
  }
...
async function gravarSyncLogLobby(
  ...
  try {
    ...
    console.error("[monitor-lobby-rico] Falha ao gravar sync_logs:", e);
  }
}
...
serve(async (req) => {
  ...
  const { mesas: mesasList, erro: mesasLoadErr } =
    await carregarMesasMonitorRico(supabase);
  if (mesasLoadErr) {
  ...
  if (mesasList.length === 0) {
    return json({
      ...
      status: "erro_config",
      erro:
        `Nenhuma mesa Rico com ID: preencha ID Rico Bet em Gestão de Estúdios (operadora_slug=${OPERADORA_SLUG})`,
    }, req, 200);
  }
  ...
  if (Array.isArray(body.rico_lobby) && body.rico_lobby.length > 0) {
    lobby = body.rico_lobby.map((g, i) => ({
      ...g,
      game_id: String(g.game_id),
      posicao: typeof g.posicao === "number" ? g.posicao : i + 1,
    }));
    paginasLidas = body.rico_paginas_lidas ?? 1;
